...
請注意！要等到使用者都使用你的 Rails 4.x app，並確保你不會降級到 Rails 3.x，才設定 `secret_key_base`。因為 cookie 簽署的方法並不向下相容。忽略 deprecation warning 使用 `secret_token` 也是沒問題的，只要你知道你自己在做什麼就好。

如果有外部應用程式或 JavaScript 需要能夠讀你的 Rails app 的簽署 session cookies，不要設定 `secret_key_base`，直到你將這些疑慮排除。

* Rails 4.0 `secret_key_base` 有設定時，會加密以 cookie 為基礎的 session。
